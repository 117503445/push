package com.wizzstudio.push.model;

/**
 * @Author: TheR1sing3un
 * @Date: 2021/09/25/13:56
 * @Description: eventKey的类型
 */
public class EventKey {

    //使用中的昵称
    public static final String NICKNAME_ABLE = "nickname_able";

    //禁用中的昵称
    public static final String NICKNAME_DISABLED= "nickname_disabled";

    //添加昵称
    public static final String NICKNAME_ADD = "nickname_add";

    //启用昵称
    public static final String NICKNAME_ENABLE = "nickname_enable";

    //禁用昵称
    public static final String NICKNAME_DISABLE = "nickname_disable";

}
